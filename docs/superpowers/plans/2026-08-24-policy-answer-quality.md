...
- [x] Write tests that require exactly 100 unique cases, at most five retrieved
...
- [x] Run `LANGSMITH_TRACING=false pytest -q tests/test_policy_answer_eval.py`
  and confirm it fails because the loader and artifacts do not exist.
- [x] Add the minimal dataclasses/loaders to `run_policy_answer.py` and generate
  the two committed JSON artifacts from the accepted reports without querying
  the DB or rerunning retrieval.
- [x] Rerun the focused test and commit the snapshot/taxonomy slice.
...
- [x] Add failing tests proving the runner passes the stored evidence directly
...
- [x] Add failing tests for distinct metrics: `answer_correct`,
...
- [x] Implement only the answer-only target, deterministic metrics, one
...
- [x] Execute the frozen 21-case diagnostic baseline and record raw answer,
  selected evidence, citation output, validator result, and taxonomy.
- [x] State one dominant causal hypothesis from those rows.
- [x] Write a failing regression test for that single behavior.
- [x] Implement the smallest shared fix; do not touch retrieval or validator
  thresholds.
- [x] Run the focused suite. Reject the change if it does not improve the
...
- [x] Run one 100-case frozen-evidence Policy LangSmith candidate only after
  Task 3 passes offline.
- [x] Measure the fixed gates: correctness 71%, support 92%, unsupported
  confident 0%; aggregate citation grounding is 78%, while the decisive
  evidence-hit slice is 94.3%. Keep the failed aggregate gate visible.
- [x] Add tests for a 20-case Calendar answer target that verifies date/event
  correctness, exact citation grounding/source, abstention, and output contract.
- [x] Run one Calendar chat experiment against the existing frozen 20 events
...
- [x] Run `LANGSMITH_TRACING=false PYTHONPATH=$PWD pytest -q`.
- [x] Run `LANGSMITH_TRACING=false RUN_DB_TESTS=1 PYTHONPATH=$PWD pytest -q tests/integration`.
